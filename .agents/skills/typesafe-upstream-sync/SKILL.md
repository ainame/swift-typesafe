...
4. Keep the submodule pin, exact reviewed SHAs, parity matrix, README, and CHANGELOG aligned. Mirror the upstream version in `TypeSafeClient.version` only when the corresponding parity is verified. Swift tags follow upstream MAJOR.MINOR with a package-owned PATCH: an upstream minor/major release starts at patch 0; an upstream patch release takes the next free Swift patch; Python-only patches may be skipped. Add the release to the `UPSTREAM.md` mapping table. Upstream generated schemas are evidence, not an instruction to copy schema bugs over working SDK behavior.
