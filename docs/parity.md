...
- The ad-hoc result builder returns a flat tuple of typed answers (or the answer itself for one question), using the same dynamic request path and typed response accessors as `QuestionSet`. Its wire keys are positional (`question_0`, `question_1`, ...). It does not return response metadata or support conditional question insertion/runtime-sized loops. Empty builders and empty score rubrics fail the existing preflight validation. Scores remain fractional `Double` values with ordered criteria.
